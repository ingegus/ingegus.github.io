    ...
    const card = `
            <div class="col-md-4 mb-4">
                <div class="card h-100">
                    <img src="${thumbnail}" class="card__img-top" alt="Imagen del artículo ${index + 1}">
                    <div class="card__body d-flex flex-column">
                        <h3 style="font-size:16px" class="card__title">${item.title}</h3>
                        <a href="${item.link}" target="_blank" rel="noopener noreferrer" class="btn btn-primary mt-auto">Leer más</a>
                    </div>
                </div>
            </div>
        `;
...
$(document).ready(function () {
    ...
    function activateNavLink() {
        ...
        $('.nav-link').each(function () {
            ...
            if (targetSection.offset().top <= scrollPosition + 10 &&
